        ...
        <div className="relative" ref={containerRef}>
            ...
            >
                <div className="flex items-center gap-2 overflow-hidden">
                    ...
                    <span className="truncate max-w-[100px]">{isAllSelected ? label : `${label} (${selectedValues.length})`}</span>
                </div>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-cyber-card border border-cyber-blue/30 shadow-xl z-50 rounded overflow-hidden animate-fade-in">
                    <div className="p-2 border-b border-cyber-muted/20 bg-cyber-bg/50 flex justify-between items-center">
                        ...
                        {!isAllSelected && <button onClick={() => onChange([])} className="text-[10px] text-cyber-blue hover:text-white underline">清除已选</button>}
                    </div>
                    <div className="max-h-60 overflow-y-auto custom-scrollbar p-1">
                        {options.map(opt => {
                            const isSelected = selectedValues.includes(opt.value);
                            return (
                                <div key={opt.value} onClick={() => toggleOption(opt.value)} className={`flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-white/5 transition-colors text-sm font-mono ${isSelected ? 'text-white' : 'text-cyber-muted'}`}>
                                    {isSelected ? <CheckSquare size={16} className="text-cyber-blue" /> : <Square size={16} className="opacity-50" />}
...
export const OrderDatabase: React.FC<OrderDatabaseProps> = ({ orders, models, onAddOrder, onUpdateOrder, onDeleteOrder }) => {
  ...
  const calculateEffectiveHours = (model: MachineModel) => {
      if (model.scheduleCalculationModule) {
          return model.steps.filter(s => s.parallelModule === model.scheduleCalculationModule).reduce((sum, s) => sum + s.estimatedHours, 0);
      } else {
  ...
  const resetForm = () => {
    ...
    setExistingStatus(MachineStatus.PLANNED); setExistingStepIndex(0); setExistingLogs([]); setExistingStepStates({}); setExistingEstimatedDate(''); setExistingOriginalDate(undefined);
  };
  ...
  const handleEditClick = (order: WorkOrder) => {
    ...
    setExistingStatus(order.status); setExistingStepIndex(order.currentStepIndex); setExistingLogs(order.logs); setExistingStepStates(order.stepStates || {}); setExistingEstimatedDate(order.estimatedCompletionDate); setExistingOriginalDate(order.originalEstimatedCompletionDate);
    setActiveTab('CREATE');
  };
  ...
  const handleExportCurrentList = () => {
      const exportData = orders.map(order => {
        ...
        return {
            ...
            "假日别": order.holidayType, "二轴头": order.axisHead || '', "刀柄规格": order.toolHolderSpec || '', "刀庫數": order.magazineCount || '', "Z軸行程": order.zAxisTravel || '', "主軸轉速": order.spindleSpeed || ''
        };
    });
    ...
    reader.onload = (evt) => {
      try {
          ...
          data.forEach((row: any) => {
              ...
              const effectiveHours = calculateEffectiveHours(model); const projected = calculateProjectedDate(new Date(startIso), effectiveHours, holidayTypeVal); const projectedISO = projected.toISOString();
              let closingIso = undefined;
              if (closingDateVal) { let d = new Date(closingDateVal); if (typeof closingDateVal === 'number') { d = new Date(Math.round((closingDateVal - 25569) * 86400 * 1000)); } if (!isNaN(d.getTime())) closingIso = d.toISOString(); }
              const newOrder: WorkOrder = { id: String(id), modelId: model.id, status: MachineStatus.PLANNED, currentStepIndex: 0, workshop: workshopVal, startDate: startIso, estimatedCompletionDate: projectedISO, originalEstimatedCompletionDate: projectedISO, businessClosingDate: closingIso, holidayType: holidayTypeVal, clientName: client, axisHead: axis, toolHolderSpec: tool, magazineCount: magazine, zAxisTravel: zAxis, spindleSpeed: spindle, stepStates: {}, logs: [] };
              onAddOrder(newOrder); successCount++;
          });
          alert(`批量导入完成。\n成功: ${successCount} 条\n失败/跳过: ${failCount} 条`);
             ...
             <div className="bg-cyber-card border border-cyber-blue/30 p-8 relative overflow-hidden shadow-neon-blue max-w-5xl mx-auto">
                ...
                {!editingOrderId && (
                    <div className="mb-8 p-4 bg-cyber-bg/40 border border-cyber-muted/20 rounded relative">
                         ...
                         <div className="flex gap-4">
                             <button onClick={handleDownloadTemplate} className="flex items-center gap-2 px-4 py-2 bg-cyber-card border border-cyber-muted/30 text-cyber-muted text-sm font-mono hover:text-white hover:border-cyber-blue transition-all"><FileDown size={16}/> 下载Excel模板</button>
                             <button onClick={triggerFileUpload} className="flex items-center gap-2 px-4 py-2 bg-cyber-card border border-cyber-muted/30 text-cyber-muted text-sm font-mono hover:text-green-400 hover:border-green-500 transition-all"><Upload size={16}/> 导入Excel数据</button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                    <div className="space-y-6">
                        ...
                        <div>
                            <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><Hash size={14}/> 机台号 (序列号) <span className="text-red-500">*</span></label>
                            <input type="text" value={machineId} onChange={(e) => setMachineId(e.target.value)} placeholder="例如: SN-2024-088" className="w-full bg-cyber-bg border border-cyber-muted/40 p-3 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm transition-shadow focus:shadow-neon-blue"/>
                        </div>
                        <div>
                            <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><Factory size={14}/> 生产车间</label>
                            <select value={workshop} onChange={(e) => setWorkshop(e.target.value)} className="w-full bg-cyber-bg border border-cyber-muted/40 p-3 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm">
                        ...
                        <div>
                            <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><User size={14}/> 客户名称</label>
                            <input type="text" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="例如: 江苏大前机床" className="w-full bg-cyber-bg border border-cyber-muted/40 p-3 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/>
                        </div>
                    </div>

                    <div className="space-y-6">
                        <div>
                            <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><Calendar size={14}/> 计划上线日期 <span className="text-red-500">*</span></label>
                            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full bg-cyber-bg border border-cyber-muted/40 p-3 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><CalendarClock size={14}/> 假日别 (计算依据)</label>
                                <select value={holidayType} onChange={(e) => setHolidayType(e.target.value as HolidayType)} className="w-full bg-cyber-bg border border-cyber-muted/40 p-3 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm">
                            ...
                            <div>
                                <label className="block text-xs font-mono text-cyber-blue mb-2 uppercase tracking-wider flex items-center gap-2"><Calendar size={14}/> 计划生产完工 <span className="text-[10px] opacity-70">(基准)</span></label>
                                <div className="relative">
                                    <input type="date" value={endDate} readOnly className="w-full bg-cyber-bg/50 border border-cyber-muted/20 p-3 text-cyber-blue focus:outline-none font-mono text-sm cursor-not-allowed font-bold" title="根据上线日期和工艺工时自动计算原始计划基准。"/>
                                    <Lock size={12} className="absolute right-3 top-3.5 text-cyber-muted opacity-50"/>
                                </div>
                            </div>
                        </div>
                         <div>
                            <label className="block text-xs font-mono text-cyber-orange mb-2 uppercase tracking-wider flex items-center gap-2"><Calendar size={14}/> 业务结关日期 (截止)</label>
                            <input type="date" value={businessDate} onChange={(e) => setBusinessDate(e.target.value)} className="w-full bg-cyber-bg border border-cyber-orange/40 p-3 text-white focus:border-cyber-orange focus:outline-none font-mono text-sm"/>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2"><label className="block text-xs font-mono text-cyber-muted mb-2 uppercase tracking-wider flex items-center gap-2 border-b border-cyber-muted/20 pb-1"><Settings size={14}/> 技术规格配置</label></div>
                            <div><label className="block text-[10px] font-mono text-cyber-blue mb-1 uppercase">二轴头</label><input type="text" value={axisHead} onChange={(e) => setAxisHead(e.target.value)} placeholder="例如: 直结式" className="w-full bg-cyber-bg border border-cyber-muted/40 p-2 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/></div>
                            <div><label className="block text-[10px] font-mono text-cyber-blue mb-1 uppercase">刀库数</label><input type="text" value={magazineCount} onChange={(e) => setMagazineCount(e.target.value)} placeholder="例如: 24T" className="w-full bg-cyber-bg border border-cyber-muted/40 p-2 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/></div>
                             <div><label className="block text-[10px] font-mono text-cyber-blue mb-1 uppercase">刀柄规格</label><input type="text" value={toolHolderSpec} onChange={(e) => setToolHolderSpec(e.target.value)} placeholder="例如: BT50" className="w-full bg-cyber-bg border border-cyber-muted/40 p-2 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/></div>
                            <div><label className="block text-[10px] font-mono text-cyber-blue mb-1 uppercase">Z轴行程</label><input type="text" value={zAxisTravel} onChange={(e) => setZAxisTravel(e.target.value)} placeholder="例如: 800mm" className="w-full bg-cyber-bg border border-cyber-muted/40 p-2 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/></div>
                            <div><label className="block text-[10px] font-mono text-cyber-blue mb-1 uppercase">主轴转速</label><input type="text" value={spindleSpeed} onChange={(e) => setSpindleSpeed(e.target.value)} placeholder="例如: 12000rpm" className="w-full bg-cyber-bg border border-cyber-muted/40 p-2 text-white focus:border-cyber-blue focus:outline-none font-mono text-sm"/></div>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left font-mono">
                        <thead>
                            <tr className="border-b border-cyber-muted/20 text-cyber-muted text-xs uppercase tracking-wider bg-cyber-bg/30">
                                <th className="p-4">机台号</th><th className="p-4">机型</th><th className="p-4">假日别</th><th className="p-4">状态</th><th className="p-4">生产车间</th><th className="p-4">计划上线</th><th className="p-4 text-cyber-orange">业务结关</th><th className="p-4 text-right">操作</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm divide-y divide-cyber-muted/10">
                            {filteredOrders.length === 0 ? (<tr><td colSpan={8} className="p-8 text-center text-cyber-muted opacity-60">无符合条件的记录</td></tr>) : (
                                filteredOrders.map(order => (
                                    <tr key={order.id} className="hover:bg-cyber-blue/5 transition-colors group">
                                        <td className="p-4 text-white group-hover:text-cyber-blue">
                                            <div className="flex items-baseline gap-0.5"><span>{order.id}</span>{order.zAxisTravel && <span className="text-xs font-normal text-cyber-muted/80">(Z{order.zAxisTravel.replace(/mm/gi, '').trim()})</span>}</div>
                                            <div className="text-[10px] text-cyber-muted mt-1 font-normal opacity-70">{[order.axisHead, order.toolHolderSpec, order.spindleSpeed, order.magazineCount].filter(Boolean).join(' / ') || '-'}</div>
                                        </td>
                                        ...
                                        <td className="p-4 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => handleEditClick(order)} className="p-1 text-cyber-blue hover:bg-cyber-blue/20 rounded transition-colors" title="编辑"><Edit size={16} /></button>
                                                <button onClick={() => onDeleteOrder(order.id)} className="p-1 text-cyber-muted hover:text-cyber-orange hover:bg-cyber-orange/20 rounded transition-colors" title="删除"><Trash2 size={16} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
